...
for iu, ti in enumerate(t_timr):
    ...
    if iu == 0:
        ...
        stringtime=str(ty) + " " + str(tmm) + " " + str(td) + " " + str(th) + " " + str(tminute) + " " + str(tsecond) + "." + str(tmicro).zfill(UTCDateTime.DEFAULT_PRECISION)
        ...
        if (t_sumr[ie] > min_threshold and t_nchr[ie] >= min_nch):
            if FlagDateTime==1:
                ...
                sf= stringtime + " " + str(t_magr[ie]) + " " + str(t_aver[ie]) + " " + str(t_sumr[ie]) + " " + str(int(t_numr[iu])) + " " + str(lat) + " " + str(lon) + " " + str(dep) + " " + str(t_nchr[ie])
            else:
