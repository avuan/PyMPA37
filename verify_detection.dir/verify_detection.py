...
'T0701': [39.98617, 16.116141, 0.882], 'T0711': [39.93571, 16.06183, 0.751], 'T0715': [39.8384, 16.0683, 1.044], 'CET2': [39.528756, 15.954618, 0.675]}

# setup station list
stations=["ORI", "SALB", "SCHR", "SIRI", "CUC", "MMN", "T0701", "T0711", "T0715", "CET2"]
...
#for jf, detection_num in enumerate(range(85,86)):
    ...
    detection_daily_otime=UTCDateTime(yy, mm, dd, hh, mmn, ss).timestamp - UTCDateTime(yy, mm, dd, 0, 0, 0, 0).timestamp
    ...
    template_daily_otime=UTCDateTime(yyt, mmt, ddt, hht, mint, sst, microsect).timestamp - UTCDateTime(yyt, mmt, ddt, 0, 0, 0, 0).timestamp
    ...
    for it, tt in enumerate(st1_temp):
        ...
        if st_cont.select(id=idchan).__nonzero__():
            ...
            tc.trim(starttime=UTCDateTime(detection_otime), endtime=UTCDateTime(detection_otime)+2*det_dur, pad=True, nearest_sample=True, fill_value=0)
        ...
        #axarray[count-1].text(det_dur*0.05, -0.9*magg, 'detection_time=' + str(detection_otime), fontsize=11)
            if count == 1:
                axarray[count-1].text(det_dur*0.65, 1.4*magg, 'Detection = ' + str(detection_num) + ' ' + 'Template = ' + str(template_num), fontsize=16)
                print("det_dur, magg == ", det_dur, magg)
