...
for day in days:
    ...
    for itemp in range(t_start, t_stop):
        ...
        # from d dictionary find minimum time to recover origin time
        ...
        # correction for origin time of event
        tdifmin = min(tdif[0:])
        tdifmax = max(tdif[0:])
        ...
        for itrig, trg in enumerate(triglist):
            ...
            # retrieve origin time from trigger using the travel time of the closest station
            if tdifmin == min_time_value:
                ...
                print("tdifmin", tdifmin, " min_time_value", min_time_value)
            ...
            if int(nch[itrig]) >= nch_min:
                ...
                # for each trigger, detrended, and filtered continuous data
                # in useful channels are trimmed and amplitude usefule to estimate 
