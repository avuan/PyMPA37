...
def stack(stall, df, tstart, npts, stdup, stddown):
    ...
    # compute tdifmin for channels used in the stack function
    # tdifmin = reference time to stack cft and to retrieve
    # the origin time of the detection
    for i, cft in enumerate(stall):
        # get station name from trace
        sta = cft.stats.station
        chan = cft.stats.channel
        net = cft.stats.network
