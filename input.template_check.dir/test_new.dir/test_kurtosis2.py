...
#stations = ['ATPI', 'ATCC', 'ATLO', 'ATPC', 'ATVO', 'FOSV', 'FRON', 'NARO', 'PARC', 'SSFR']
stations = ['NRCA']
networks = ['IV', 'MN']
            ...
            for i in range(0, 570):
                ...
                try:
                    if os.path.isfile(inpfile):
                        ...
                        # third test
                        spike = 0
                        astd = 5 * np.std(tr.data)

                        if amaxa > astd:
                            print('spike detetcted')
                            spike = 1
                        
                        #tre = envelope(tr.data)
                        k = kurtosis(abs(tr.data), axis=0, fisher=True, bias=True, nan_policy='propagate')
                        kstring = 'kurt(Pearson) = ' + str(k)[0:6] + ' ind1 = ' + str(indm) + ' ind2 = ' + str(indm95)
                        #print(inpfile, k, indm, indm95)

                        if k<=0.8 or indm==0 or indm95==0 or spike==1: 
                            call(['cp', inpfile, './bad'])
                            print(inpfile, k, indm, indm95)
                        elif k>0.8 and indm==1 and indm95==1 and spike==0:
                        ...
                        if FlagPrint==1:
                            ...
                            if k<=0.8 or indm==0 or indm95==0 or spike==1:
                                #ax.plot(t, abs(tr.data), 'r')
                                ax.plot(t, tr.data, 'r')
                            elif k>0.8 and indm==1 and indm95==1 and spike==0:
                                #ax.plot(t, abs(tr.data), 'k')
                                ax.plot(t, tr.data, 'k')
