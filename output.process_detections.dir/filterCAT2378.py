...
for it, tt in enumerate(times):
    ...
    strcc_ave0 = str(cc_ave0[indeces[0][index]])
    strcc_sum0 = str(cc_sum0[indeces[0][index]])
    strnch = str(nch[indeces[0][index]])
    # print("it", it)
    seq = (strtemp, ttdet, strmt, strcc_ave, strcc_sum, strcc_ave0, strcc_sum0, strnch)
    dets = " ".join(seq)
    # print("dets == ", dets)
    det[it] = dets
...
t_ave0 = np.empty(num_lines1)
t_sum0 = np.empty(num_lines1)
...
for ind1, line in enumerate(f1):
    ...
    t_ave0[ind1] = columns1[5]
    t_sum0[ind1] = columns1[6]
    t_nch[ind1] = columns1[7]
...
t_aver0 = np.empty(num_lines1)
t_sumr0 = np.empty(num_lines1)
...
for irr, jj in enumerate(ireorder):
    ...
    t_aver[irr] = t_ave[jj]
    t_sumr[irr] = t_sum[jj]
    t_aver0[irr] = t_ave0[jj]
    t_sumr0[irr] = t_sum0[jj]
    t_nchr[irr] = t_nch[jj]
...
for iu, ti in enumerate(t_timr):
    ...
    if iu == 0:
        ...
        ind_winners1 = np.argwhere(t_sumr[indeces1[0]] == np.max(t_sumr[indeces[0]]))
        if len(ind_winners1) > 1:
            index1 = np.argmax(t_sumr0[indeces1[0]])
        else:
